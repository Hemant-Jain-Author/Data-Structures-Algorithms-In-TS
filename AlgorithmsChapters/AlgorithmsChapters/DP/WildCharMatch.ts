function matchExp(exp: string, str: string): boolean {
	return matchExpUtil(exp.split(''), str.split(''), 0, 0);
}
...
function matchExpDP(exp: string, str: string): boolean {
	return matchExpUtilDP(exp.split(''), str.split(''), exp.length, str.length);
}

function matchExpUtilDP(exp: string[], str: string[], m: number, n: number): boolean {
	const lookup: boolean[][] = new Array(m + 1);
	for (let i = 0; i < lookup.length; i++) {
		lookup[i] = new Array(n + 1).fill(false);
	}
	lookup[0][0] = true;
